...
function configuredInstanceValue(ownerDocument: Document, property: string, fallback: number) {
  const ownerWindow = ownerDocument.defaultView
...
function outlineInstance(endpoint: GlyphEndpoint): OutlineInstance {
  ...
  const computedWeight = numericFontWeight(endpoint.style.fontWeight, role === 'sans' ? 500 : 600)
  return {
    role,
    ...
    opticalSize:
      role === 'serif'
        ...
        : 0,
    ...(endpoint.style.fontFileIndex === undefined
      ? {}
      : { fontFileIndex: endpoint.style.fontFileIndex }),
  }
}
...
export async function prepareFontMorph(key?: string): Promise<void> {
  ...
  const target = counterpartOutlineInstance(
    targetRole,
    captured.element.ownerDocument,
