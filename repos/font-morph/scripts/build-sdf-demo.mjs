...
const pairFiles = {
  ...
  japanese: [{ file: 'noto-sans-jp-400-outline.ttf' }, { file: 'noto-serif-jp-600-outline.ttf' }],
}
  ...
  const characters = [
    ...new Set(
      Object.values(samples)
        .filter((sample) => sample.pair === id)
        .flatMap((sample) => [...sample.text]),
    ),
  ]
...
console.log(
  `wrote distance-field demo data to ${outputFile} in ${(performance.now() - started).toFixed(1)}ms`,
)
